...
def main(argv):
    ...
    checkpointer = StreamingCheckpointer(
        FLAGS.checkpointer, FLAGS.logger.output_dir,
    ...
    with mesh:
        ...
        # TODO: check if optimizer replace, if so replace optimizer and set step to 0 from checkpoint
        ...
        # Initialize wandb
        wandb.init(entity='harvardml', project='EasyLM-taylor-multigpu', config=flags_config_dict)  # Replace with your project name
        ...
        for global_step in tqdm(range(FLAGS.total_steps)):
            rng = next_rng()
        ...
        if FLAGS.eval_freq and FLAGS.eval_steps > 0:
            ...
            for _ in range(FLAGS.eval_steps):
                ...
                sharded_rng, eval_metrics = sharded_eval_step(
                    eval_params, eval_batch, sharded_rng
                )
                eval_metric_list.append(eval_metrics)
            ...
            wandb.log(avg_metrics)
            # if FLAGS.weight_average:
            #     run_evaluation(ema, eval_dataset, FLAGS.total_steps, dataset_metrics['total_tokens'])
            # else:
            #     run_evaluation(new_params, eval_dataset, FLAGS.total_steps, dataset_metrics['total_tokens'])
